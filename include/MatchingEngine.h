...
public:
    MatchingEngine(std::map<int, std::deque<Order>> buyOrders, std::map<int, std::deque<Order>> sellOrders);

    std::vector<Trade> processOrder(Side side, int price, int quantity);
    void printBook();
    const std::map<int, std::deque<Order>>& getBuyOrders() const;
    const std::map<int, std::deque<Order>>& getSellOrders() const;

private:
    uint64_t nextOrderId {1};
    std::map<int, std::deque<Order>> buyOrders;
